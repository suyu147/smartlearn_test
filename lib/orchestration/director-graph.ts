...
async function runAgentGeneration(
  ...
  const storeScenes = (state.storeState.scenes ?? []) as Array<{ id: string; type: string }>;
  const currentScene = state.storeState.currentSceneId
    ? storeScenes.find((s) => s.id === (state.storeState.currentSceneId as string))
    : undefined;
      ...
      if (chunk.type === 'delta') {
        ...
        for (const entry of parseResult.ordered) {
          if (entry.type === 'text') {
            ...
            if (!ac) continue;
            if (!ac.actionName || !effectiveActions.includes(ac.actionName)) {
              log.warn(
            ...
            if (ac.actionName.startsWith('wb_')) {
              whiteboardActions.push({
                actionType: ac.actionName,
                actionName: ac.actionName as WhiteboardActionRecord['actionName'],
                agentId,
                agentName: agentConfig.name,
                params: ac.params,
                description: `${agentConfig.name}: ${ac.actionName}`,
                timestamp: Date.now(),
              });
            }
            write({
...
export function buildInitialState(
  ...
  return {
    ...
    currentAgentId: null,
    turnCount,
    agentResponses: (Array.isArray(incoming?.agentResponses) ? incoming.agentResponses : []) as AgentTurnSummary[],
