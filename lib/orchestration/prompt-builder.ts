...
export function buildStructuredPrompt(
  ...
  userProfile?: string | { nickname?: string; bio?: string },
  agentResponses?: AgentTurnSummary[],
): string {
  // Determine current scene type for action filtering
  const scenes = (storeState.scenes || []) as Array<{ id: string; type: string; title?: string; content?: Record<string, unknown> }>;
  const currentScene = storeState.currentSceneId
    ? scenes.find((s) => s.id === storeState.currentSceneId)
    : undefined;
  ...
  const studentProfileSection =
    (typeof userProfile === 'object' && (userProfile?.nickname || userProfile?.bio))
      ? `\n# Student Profile
...
- Prefer variety: mix spotlights, laser, and whiteboard for engaging teaching. Don't use the same action type repeatedly.`
  ...
  const roleGuideline = ROLE_GUIDELINES[agentConfig.role ?? 'student'] || ROLE_GUIDELINES.student;

  // Build language constraint from stage language
  const courseLanguage = (storeState.stage as Record<string, unknown> | undefined)?.language as string | undefined;
...
${buildLengthGuidelines(agentConfig.role ?? 'student')}
...
${buildWhiteboardGuidelines(agentConfig.role ?? 'student')}
  ...
  for (const record of ledger) {
    switch (record.actionName) {
      ...
      case 'wb_draw_text': {
        ...
        const content = String(p.content || '').slice(0, 40);
        const x = p.x ?? '?';
        const y = p.y ?? '?';
        const w = p.width ?? 400;
      ...
      case 'wb_draw_shape': {
        const p = record.params ?? {};
        const shapeType = p.type || p.shape || 'rectangle';
        const x = p.x ?? '?';
        const y = p.y ?? '?';
        const w = p.width ?? 100;
        const h = p.height ?? 100;
      ...
      case 'wb_draw_chart': {
        const p = record.params ?? {};
        const chartType = p.chartType || p.type || 'bar';
        const labels = Array.isArray(p.labels)
          ? p.labels
          : (p.data as Record<string, unknown>)?.labels;
        const x = p.x ?? '?';
        const y = p.y ?? '?';
        const w = p.width ?? 350;
      ...
      case 'wb_draw_latex': {
        const p = record.params ?? {};
        const latex = String(p.latex || '').slice(0, 40);
        const x = p.x ?? '?';
        const y = p.y ?? '?';
        const w = p.width ?? 400;
        const h = p.height ?? 80;
      ...
      case 'wb_draw_line': {
        ...
        const ex = p.endX ?? '?';
        const ey = p.endY ?? '?';
        const pts = p.points as string[] | undefined;
...
function buildStateContext(storeState: StatelessChatRequest['storeState']): string {
  const stage = storeState.stage as Record<string, unknown> | undefined;
  const scenes = (storeState.scenes || []) as Array<{ id: string; type: string; title?: string; content?: Record<string, unknown> }>;
  const currentSceneId = storeState.currentSceneId as string | undefined;
  const mode = storeState.mode as string | undefined;
  const whiteboardOpen = storeState.whiteboardOpen as boolean | undefined;
  ...
  if (currentSceneId) {
    ...
    if (currentScene) {
      ...
      if (currentScene.content?.type === 'slide') {
        const canvas = currentScene.content.canvas as { elements: unknown[] } | undefined;
        if (canvas) {
      ...
      if (currentScene.content?.type === 'quiz') {
        const questions = currentScene.content.questions as Array<{ type: string; question: string }> | undefined;
        if (questions) {
  ...
  const stageWhiteboard = stage?.whiteboard as Array<{ elements: unknown[] }> | undefined;
  if (stageWhiteboard && stageWhiteboard.length > 0) {
    const lastWb = stageWhiteboard[stageWhiteboard.length - 1];
    const wbElements = lastWb.elements || [];
    lines.push(
      `Whiteboard (last of ${stageWhiteboard.length}, ${wbElements.length} elements):\n${summarizeElements(wbElements)}`,
    );
  }
  ...
  return messages
    ...
    .map((msg) => {
      if (msg.role === 'assistant') {
        ...
        const msgAgentId = msgMeta?.agentId as string | undefined;
        ...
        if (currentAgentId && msgAgentId && msgAgentId !== currentAgentId) {
          const agentName = (msgMeta?.senderName as string) || msgAgentId;
          return {
            role: 'user' as const,
      ...
      const senderMeta = msg.metadata as Record<string, unknown> | undefined;
      const senderName = senderMeta?.senderName as string | undefined;
